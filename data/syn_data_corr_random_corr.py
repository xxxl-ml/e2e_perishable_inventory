'''
generate synthetic data with correlated demand and random lead time, and correlated demand and lead time
'''
...
"""configuration"""
# data save path
data_path = "data/syn_data_corr_random_corr/"
...
for seed in range(20):
    ...
    phi_x = 0.8
    epsilon_std_x = 1 * np.sqrt((1 - phi_x**2) * 0.25)  # 0.3

    # assuming basic lead time is AR(1) process with mean 3
    phi_L = 0.8
    epsilon_std_L = np.sqrt(0.25) # 0.5
    bias_L = 3 * (1 - phi_L)

    # a common shock for demand and lead time, also following AR(1) process with zero mean
    phi_shock = 0.8
    shock_std = 1.0
    ...
    # basic lead time
    basic_leadtime = np.zeros((dc_num, sku_num, total_len))
    basic_leadtime[:, :, 0] = np.random.randint(1, 10, size=(dc_num, sku_num))
    for t in range(1, total_len):
        epsilon = np.random.normal(0, epsilon_std_L, size=(dc_num, sku_num))
        basic_leadtime[:, :, t] = bias_L + phi_L * basic_leadtime[:, :, t-1] + epsilon

    # unobservable common shock for demand and lead time
    shock = np.zeros((dc_num, sku_num, total_len))
    shock[:, :, 0] = np.random.normal(0, shock_std / (1 - phi_shock**2), size=(dc_num, sku_num))
    for t in range(1, total_len):
        epsilon = np.random.normal(0, shock_std, size=(dc_num, sku_num))
        shock[:, :, t] = phi_shock * shock[:, :, t-1] + epsilon
    ...
    for dc_idx in range(dc_num):
        for sku_idx in range(sku_num):
            for t in range(total_len):
                demand[dc_idx, sku_idx, t] = np.exp(dc_sku_fea[dc_idx, sku_idx, t] - 0.5) + 2 * (dc_fea[dc_idx, t] + sku_fea[sku_idx, t] - 1)**2 + np.abs(com_fea[t] - 0.5) + noise_D[dc_idx, sku_idx, t] + np.sqrt((1 - phi_x**2) * 0.75) * shock[dc_idx, sku_idx, t]
                leadtime[dc_idx, sku_idx, t] = basic_leadtime[dc_idx, sku_idx, t] + np.sqrt(0.75) * shock[dc_idx, sku_idx, t]
